...
CREATE LOGIN testing WITH PASSWORD = 'S3cur3!Passw0rd@';
GO
...
VALUES
	(1, 'Intensificación', 'Si un estudiante aumenta la cantidad de agua en el vaso durante su prueba del agua, es de Intensificaci�n'),
	(2, 'Transformación', 'Si un estudiante cambia el sabor del agua durante su prueba del agua es un Transformador'),
	(3, 'Materialización', 'Si un estudiante hace aparecer impurezas en el agua del vaso durante su prueba ellos son Materializaci�n'),
	...
	(6, 'Especialización', 'Si un estudiante hace alg�n otro efecto durante su prueba del agua, son Especialistas')
...
BEGIN
	...
	IF @Clave <> @ClaveConfirmar
		BEGIN
			SELECT 400 AS StatusCode, 'Las Contrase�as NO Coinciden' AS StatusMessage
			RETURN
		END
...
BEGIN
    ...
    IF @Hash <> @hashProvidedPassword OR @Hash IS NULL OR @hashProvidedPassword IS NULL
		BEGIN
			SELECT 400 AS StatusCode, 'Usuario o Contrase�a Incorrecta' AS StatusMessage
			RETURN
		END
	...
	BEGIN TRY
		...
		SELECT 201 AS StatusCode, 'Inicio de Sesi�n Exitoso.' AS StatusMessage
    END TRY
    BEGIN CATCH
		ROLLBACK TRANSACTION
